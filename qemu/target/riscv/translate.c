...
static void generate_exception(DisasContext *ctx, int excp)
{
    ...
    tcg_gen_movi_tl(tcg_ctx, tcg_ctx->riscv_cpu_pc, ctx->base.pc_next);
...
static void generate_exception_mbadaddr(DisasContext *ctx, int excp)
{
    ...
    tcg_gen_movi_tl(tcg_ctx, tcg_ctx->riscv_cpu_pc, ctx->base.pc_next);
    tcg_gen_st_tl(tcg_ctx, tcg_ctx->riscv_cpu_pc, tcg_ctx->cpu_env, offsetof(CPURISCVState, badaddr));
    TCGv_i32 helper_tmp = tcg_const_i32(tcg_ctx, excp);
    gen_helper_raise_exception(tcg_ctx, tcg_ctx->cpu_env, helper_tmp);
    tcg_temp_free_i32(tcg_ctx, helper_tmp);
    ctx->base.is_jmp = DISAS_NORETURN;
...
static void gen_goto_tb(DisasContext *ctx, int n, target_ulong dest)
{
    ...
    if (use_goto_tb(ctx, dest)) {
        ...
        tcg_gen_movi_tl(tcg_ctx, tcg_ctx->riscv_cpu_pc, dest);
...
static inline void gen_get_gpr(DisasContext *ctx, TCGv t, int reg_num)
{
    ...
    if (reg_num == 0) {
        ...
        tcg_gen_mov_tl(tcg_ctx, t, tcg_ctx->riscv_cpu_gpr[reg_num]);
    }
}
 ...
 * $zero
 */
static inline void gen_set_gpr(DisasContext *ctx, int reg_num_dst, TCGv t)
{
    TCGContext *tcg_ctx = ctx->uc->tcg_ctx;

    if (reg_num_dst != 0) {
        tcg_gen_mov_tl(tcg_ctx, tcg_ctx->riscv_cpu_gpr[reg_num_dst], t);
    }
}

static void gen_mulhsu(DisasContext *ctx, TCGv ret, TCGv arg1, TCGv arg2)
{
    TCGContext *tcg_ctx = ctx->uc->tcg_ctx;
    ...
    uint32_t rs2, int rm, uint64_t min)
{
    ...
    case 0: /* fsgnj */
        if (rs1 == rs2) { /* FMOV */
            tcg_gen_mov_i64(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->riscv_cpu_fpr[rs1]);
        } else {
            tcg_gen_deposit_i64(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->riscv_cpu_fpr[rs2], tcg_ctx->riscv_cpu_fpr[rs1],
                                0, min == INT32_MIN ? 31 : 63);
        }
        break;
    case 1: /* fsgnjn */
        if (rs1 == rs2) { /* FNEG */
            tcg_gen_xori_i64(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->riscv_cpu_fpr[rs1], min);
        } else {
            TCGv_i64 t0 = tcg_temp_new_i64(tcg_ctx);
            tcg_gen_not_i64(tcg_ctx, t0, tcg_ctx->riscv_cpu_fpr[rs2]);
            tcg_gen_deposit_i64(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], t0, tcg_ctx->riscv_cpu_fpr[rs1],
                                0, min == INT32_MIN ? 31 : 63);
    ...
    case 2: /* fsgnjx */
        if (rs1 == rs2) { /* FABS */
            tcg_gen_andi_i64(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->riscv_cpu_fpr[rs1], ~min);
        } else {
            TCGv_i64 t0 = tcg_temp_new_i64(tcg_ctx);
            tcg_gen_andi_i64(tcg_ctx, t0, tcg_ctx->riscv_cpu_fpr[rs2], min);
            tcg_gen_xor_i64(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->riscv_cpu_fpr[rs1], t0);
            tcg_temp_free_i64(tcg_ctx, t0);
        }
        break;
    default:
                    ...
                    target_ulong imm)
{
    ...
    if (rd != 0) {
        tcg_gen_movi_tl(tcg_ctx, tcg_ctx->riscv_cpu_gpr[rd], ctx->pc_succ_insn);
    }
                     ...
                     int rd, int rs1, target_long imm)
{
    ...
    switch (opc) {
    case OPC_RISC_JALR:
        gen_get_gpr(ctx, tcg_ctx->riscv_cpu_pc, rs1);
        tcg_gen_addi_tl(tcg_ctx, tcg_ctx->riscv_cpu_pc, tcg_ctx->riscv_cpu_pc, imm);
        tcg_gen_andi_tl(tcg_ctx, tcg_ctx->riscv_cpu_pc, tcg_ctx->riscv_cpu_pc, (target_ulong)-2);

        if (!riscv_has_ext(env, RVC)) {
            misaligned = gen_new_label(tcg_ctx);
            tcg_gen_andi_tl(tcg_ctx, t0, tcg_ctx->riscv_cpu_pc, 0x2);
        ...
        int rs1, target_long imm)
{
    ...
    switch (opc) {
    case OPC_RISC_FLW:
        tcg_gen_qemu_ld_i64(ctx->uc, tcg_ctx->riscv_cpu_fpr[rd], t0, ctx->mem_idx, MO_TEUL);
        /* RISC-V requires NaN-boxing of narrower width floating point values */
        tcg_gen_ori_i64(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->riscv_cpu_fpr[rd], 0xffffffff00000000ULL);
        break;
    case OPC_RISC_FLD:
        tcg_gen_qemu_ld_i64(ctx->uc, tcg_ctx->riscv_cpu_fpr[rd], t0, ctx->mem_idx, MO_TEQ);
        break;
    default:
        gen_exception_illegal(ctx);
        break;
        ...
        int rs2, target_long imm)
{
    ...
    switch (opc) {
    case OPC_RISC_FSW:
        tcg_gen_qemu_st_i64(ctx->uc, tcg_ctx->riscv_cpu_fpr[rs2], t0, ctx->mem_idx, MO_TEUL);
        break;
    case OPC_RISC_FSD:
        tcg_gen_qemu_st_i64(ctx->uc, tcg_ctx->riscv_cpu_fpr[rs2], t0, ctx->mem_idx, MO_TEQ);
        break;
    default:
        gen_exception_illegal(ctx);
        break;
                      ...
                      int rd, int rs1, int rs2)
{
    ...
    case OPC_RISC_LR:
        ...
        tcg_gen_qemu_ld_tl(ctx->uc, tcg_ctx->load_val, src1, ctx->mem_idx, mop);
        if (aq) {
            tcg_gen_mb(tcg_ctx, TCG_MO_ALL | TCG_BAR_LDAQ);
        }
        tcg_gen_mov_tl(tcg_ctx, tcg_ctx->load_res, src1);
        gen_set_gpr(ctx, rd, tcg_ctx->load_val);
        break;
                         ...
                         int rs1, int rs2, int rs3, int rm)
{
    ...
    case OPC_RISC_FMADD_S:
        gen_set_rm(ctx, rm);
        gen_helper_fmadd_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1],
                           tcg_ctx->riscv_cpu_fpr[rs2], tcg_ctx->riscv_cpu_fpr[rs3]);
        break;
    case OPC_RISC_FMADD_D:
        gen_set_rm(ctx, rm);
        gen_helper_fmadd_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1],
                           tcg_ctx->riscv_cpu_fpr[rs2], tcg_ctx->riscv_cpu_fpr[rs3]);
        break;
    default:
        gen_exception_illegal(ctx);
                         ...
                         int rs1, int rs2, int rs3, int rm)
{
    ...
    case OPC_RISC_FMSUB_S:
        gen_set_rm(ctx, rm);
        gen_helper_fmsub_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1],
                           tcg_ctx->riscv_cpu_fpr[rs2], tcg_ctx->riscv_cpu_fpr[rs3]);
        break;
    case OPC_RISC_FMSUB_D:
        gen_set_rm(ctx, rm);
        gen_helper_fmsub_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1],
                           tcg_ctx->riscv_cpu_fpr[rs2], tcg_ctx->riscv_cpu_fpr[rs3]);
        break;
    default:
        gen_exception_illegal(ctx);
                          ...
                          int rs1, int rs2, int rs3, int rm)
{
    ...
    case OPC_RISC_FNMSUB_S:
        gen_set_rm(ctx, rm);
        gen_helper_fnmsub_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1],
                            tcg_ctx->riscv_cpu_fpr[rs2], tcg_ctx->riscv_cpu_fpr[rs3]);
        break;
    case OPC_RISC_FNMSUB_D:
        gen_set_rm(ctx, rm);
        gen_helper_fnmsub_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1],
                            tcg_ctx->riscv_cpu_fpr[rs2], tcg_ctx->riscv_cpu_fpr[rs3]);
        break;
    default:
        gen_exception_illegal(ctx);
                          ...
                          int rs1, int rs2, int rs3, int rm)
{
    ...
    case OPC_RISC_FNMADD_S:
        gen_set_rm(ctx, rm);
        gen_helper_fnmadd_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1],
                            tcg_ctx->riscv_cpu_fpr[rs2], tcg_ctx->riscv_cpu_fpr[rs3]);
        break;
    case OPC_RISC_FNMADD_D:
        gen_set_rm(ctx, rm);
        gen_helper_fnmadd_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1],
                            tcg_ctx->riscv_cpu_fpr[rs2], tcg_ctx->riscv_cpu_fpr[rs3]);
        break;
    default:
        gen_exception_illegal(ctx);
                         ...
                         int rs1, int rs2, int rm)
{
    ...
    case OPC_RISC_FADD_S:
        gen_set_rm(ctx, rm);
        gen_helper_fadd_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
        break;
    case OPC_RISC_FSUB_S:
        gen_set_rm(ctx, rm);
        gen_helper_fsub_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
        break;
    case OPC_RISC_FMUL_S:
        gen_set_rm(ctx, rm);
        gen_helper_fmul_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
        break;
    case OPC_RISC_FDIV_S:
        gen_set_rm(ctx, rm);
        gen_helper_fdiv_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
        break;
    case OPC_RISC_FSQRT_S:
        gen_set_rm(ctx, rm);
        gen_helper_fsqrt_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
        break;
    ...
    case OPC_RISC_FMIN_S:
        ...
        switch (rm) {
        case 0x0:
            gen_helper_fmin_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
            break;
        case 0x1:
            gen_helper_fmax_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FEQ_S:
        ...
        switch (rm) {
        case 0x0:
            gen_helper_fle_s(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
            break;
        case 0x1:
            gen_helper_flt_s(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
            break;
        case 0x2:
            gen_helper_feq_s(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FCVT_W_S:
        ...
        case 0: /* FCVT_W_S */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_w_s(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
        case 1: /* FCVT_WU_S */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_wu_s(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
#if defined(TARGET_RISCV64)
        case 2: /* FCVT_L_S */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_l_s(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
        case 3: /* FCVT_LU_S */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_lu_s(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
#endif
        default:
            goto do_illegal;
    ...
    case OPC_RISC_FCVT_S_W:
        ...
        case 0: /* FCVT_S_W */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_s_w(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
        case 1: /* FCVT_S_WU */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_s_wu(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
#if defined(TARGET_RISCV64)
        case 2: /* FCVT_S_L */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_s_l(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
        case 3: /* FCVT_S_LU */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_s_lu(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
#endif
        default:
            goto do_illegal;
    ...
    case OPC_RISC_FMV_X_S:
        ...
        case 0: /* FMV */
#if defined(TARGET_RISCV64)
            tcg_gen_ext32s_tl(tcg_ctx, t0, tcg_ctx->riscv_cpu_fpr[rs1]);
#else
            tcg_gen_extrl_i64_i32(tcg_ctx, t0, tcg_ctx->riscv_cpu_fpr[rs1]);
#endif
            break;
        case 1:
            gen_helper_fclass_s(tcg_ctx, t0, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FMV_S_X:
        ...
        gen_get_gpr(ctx, t0, rs1);
#if defined(TARGET_RISCV64)
        tcg_gen_mov_i64(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], t0);
#else
        tcg_gen_extu_i32_i64(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], t0);
#endif
        tcg_temp_free(tcg_ctx, t0);
        break;
    ...
    case OPC_RISC_FADD_D:
        gen_set_rm(ctx, rm);
        gen_helper_fadd_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
        break;
    case OPC_RISC_FSUB_D:
        gen_set_rm(ctx, rm);
        gen_helper_fsub_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
        break;
    case OPC_RISC_FMUL_D:
        gen_set_rm(ctx, rm);
        gen_helper_fmul_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
        break;
    case OPC_RISC_FDIV_D:
        gen_set_rm(ctx, rm);
        gen_helper_fdiv_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
        break;
    case OPC_RISC_FSQRT_D:
        gen_set_rm(ctx, rm);
        gen_helper_fsqrt_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
        break;
    ...
    case OPC_RISC_FMIN_D:
        ...
        switch (rm) {
        case 0:
            gen_helper_fmin_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
            break;
        case 1:
            gen_helper_fmax_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FCVT_S_D:
        ...
        case 1:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_s_d(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FCVT_D_S:
        ...
        case 0:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_d_s(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FEQ_D:
        ...
        switch (rm) {
        case 0:
            gen_helper_fle_d(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
            break;
        case 1:
            gen_helper_flt_d(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
            break;
        case 2:
            gen_helper_feq_d(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1], tcg_ctx->riscv_cpu_fpr[rs2]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FCVT_W_D:
        ...
        case 0:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_w_d(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
        case 1:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_wu_d(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
#if defined(TARGET_RISCV64)
        case 2:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_l_d(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
        case 3:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_lu_d(tcg_ctx, t0, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
#endif
        default:
            goto do_illegal;
    ...
    case OPC_RISC_FCVT_D_W:
        ...
        case 0:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_d_w(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
        case 1:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_d_wu(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
#if defined(TARGET_RISCV64)
        case 2:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_d_l(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
        case 3:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_d_lu(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
#endif
        default:
            goto do_illegal;
    ...
    case OPC_RISC_FMV_X_D:
        ...
        switch (rm) {
        case 0: /* FMV */
            gen_set_gpr(ctx, rd, tcg_ctx->riscv_cpu_fpr[rs1]);
            break;
        case 1:
            t0 = tcg_temp_new(tcg_ctx);
            gen_helper_fclass_d(tcg_ctx, t0, tcg_ctx->riscv_cpu_fpr[rs1]);
            gen_set_gpr(ctx, rd, t0);
            tcg_temp_free(tcg_ctx, t0);
            break;
        default:
    ...
    case OPC_RISC_FMV_D_X:
        t0 = tcg_temp_new(tcg_ctx);
        gen_get_gpr(ctx, t0, rs1);
        tcg_gen_mov_tl(tcg_ctx, tcg_ctx->riscv_cpu_fpr[rd], t0);
        tcg_temp_free(tcg_ctx, t0);
        break;
#endif
                      ...
                      int rd, int rs1, int csr)
{
    ...
    if ((opc == OPC_RISC_ECALL) && ((csr >> 5) == 9)) {
        ...
        gen_helper_riscv_tlb_flush(tcg_ctx, tcg_ctx->cpu_env);
        return;
    }
#endif
    ...
    case OPC_RISC_ECALL:
        ...
        case 0x102: /* SRET */
            if (riscv_has_ext(env, RVS)) {
                gen_helper_sret(tcg_ctx, tcg_ctx->riscv_cpu_pc, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_pc);
                tcg_gen_exit_tb(tcg_ctx, NULL, 0); /* no chaining */
                ctx->base.is_jmp = DISAS_NORETURN;
            } else {
                gen_exception_illegal(ctx);
        ...
        case 0x202: /* HRET */
            ...
            break;
        case 0x302: /* MRET */
            gen_helper_mret(tcg_ctx, tcg_ctx->riscv_cpu_pc, tcg_ctx->cpu_env, tcg_ctx->riscv_cpu_pc);
            tcg_gen_exit_tb(tcg_ctx, NULL, 0); /* no chaining */
            ctx->base.is_jmp = DISAS_NORETURN;
            break;
        case 0x7b2: /* DRET */
            ...
            break;
        case 0x105: /* WFI */
            tcg_gen_movi_tl(tcg_ctx, tcg_ctx->riscv_cpu_pc, ctx->pc_succ_insn);
...
static void decode_RV32_64C1(CPURISCVState *env, DisasContext *ctx)
{
    ...
    case 3:
        if (rd_rs1 == 2) {
            ...
            tcg_gen_movi_tl(tcg_ctx, tcg_ctx->riscv_cpu_gpr[rd_rs1],
...
static void decode_RV32_64G(CPURISCVState *env, DisasContext *ctx)
{
    ...
    case OPC_RISC_LUI:
        ...
        tcg_gen_movi_tl(tcg_ctx, tcg_ctx->riscv_cpu_gpr[rd], sextract64(ctx->opcode, 12, 20) << 12);
        break;
    case OPC_RISC_AUIPC:
        if (rd == 0) {
            break; /* NOP */
                                      ...
                                      const CPUBreakpoint *bp)
{
    ...
    tcg_gen_movi_tl(ctx->uc->tcg_ctx, tcg_ctx->riscv_cpu_pc, ctx->base.pc_next);
    ctx->base.is_jmp = DISAS_NORETURN;
    gen_exception_debug(ctx);
...
void riscv_translate_init(struct uc_struct *uc)
{
    ...
    tcg_ctx->riscv_cpu_gpr[0] = NULL;

    for (int i = 1; i < 32; i++) {
        tcg_ctx->riscv_cpu_gpr[i] = tcg_global_mem_new(tcg_ctx, tcg_ctx->cpu_env,
            offsetof(CPURISCVState, gpr[i]), riscv_int_regnames[i]);
    }

    for (int i = 0; i < 32; i++) {
        tcg_ctx->riscv_cpu_fpr[i] = tcg_global_mem_new_i64(tcg_ctx, tcg_ctx->cpu_env,
            offsetof(CPURISCVState, fpr[i]), riscv_fpr_regnames[i]);
    }

    tcg_ctx->riscv_cpu_pc = tcg_global_mem_new(tcg_ctx, tcg_ctx->cpu_env, offsetof(CPURISCVState, pc), "pc");
